...
export function AccountList() {
  const { data: accounts, isLoading: loading, error } = useAccounts()  // No userId - open-source public/mock
  ...
  accounts.forEach(account => {
    if (account.parent_id && accountMap.has(account.parent_id)) {
      accountMap.get(account.parent_id)!.children!.push(accountMap.get(account.id)!)
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        ...
        <div className="border border-white/20 p-4">
          <div className="text-2xl font-bold">
            ${accounts.reduce((sum, a) => sum + a.revenue, 0) / 1000000}M
          </div>
